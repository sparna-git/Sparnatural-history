...
export class SparnaturalQuerySummaryComponent extends HTMLComponent {
  ...
  render(): this {
    this.#initLang();
    this.querySummary = this.formatQuerySummary(
      this.queryJson,
      this.specProvider
    );
    this.addPaddings();
  ...
  private formatChildsItems(
    ...
    if (childrens.length > 0) {
      ...
      childrens.forEach((branch: any) => {
        ...
        if (branch.line.criterias.length > 0) {
          let nbValue = 0;
          branch.line.criterias.forEach((criteria: any) => {
            if (nbValue > 0) {
              selectedValues += ", ";
            }
            selectedValues += `<span class="selectedValue">${criteria.label}</span>`;
            nbValue++;
          });
          if (branch.line.criterias.length > 1) {
        ...
        if (branch.children && branch.children.length > 0) {
          htmlLI +=
            this.formatChildsItems(branch.children, variables, false) + "</li>";
        }
        htmlUL += htmlLI;
      });
      htmlUL += `</ul>`;
  ...
  private formatQuerySummary(
    ...
    summary =
      ...
      this.formatChildsItems(queryJson.branches, reformattedArray) +
      "</div>";
